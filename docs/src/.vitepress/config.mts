...
export default defineConfig({
  ...
  head: [
    ...
    // HTMX runtime — for inlining live HTMX.jl gallery fragments via
    // `<div hx-get="…" hx-trigger="load">` placeholders. HTMX requests
    // carry HX-Request: true automatically, so the web app's routes
    // return the bare body fragment that drops into the docs page
    // directly.
 ...
 * real width to lay out its card grid. */
...
@media (min-width: 960px) {
  ...
  vite: {
    ...
    server: {
      ...
      host: true,
      proxy: {
        // Live HTMX.jl gallery embedding (dev only).
        // `<div hx-get="/live-htmx/…">` forwards to the running web app
        // on :8102 so the docs page shows live state. In production,
        // point the same path at recordings produced by `record!` —
        // same markdown source. Override target via `HTMX_DEV_TARGET`.
        '/live-htmx': {
          target: process.env.HTMX_DEV_TARGET || 'http://localhost:8102',
          changeOrigin: true,
          rewrite: (path) => path.replace(/^\/live-htmx/, ''),
        }
      }
    },
    optimizeDeps: {
