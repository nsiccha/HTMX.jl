...
@testset "HTMX.HyperscriptString children move to _ attribute" begin
    # When a HyperscriptString is the sole child, it becomes the _ attribute
    hs = HTMX.HyperscriptString("on click toggle .active")
    node = h.button(hs)
    rendered = html(node)
    @test occursin("_=", rendered)
    @test occursin("on click toggle .active", rendered)
    # Should not appear as a text child
    @test !occursin(">on click toggle .active<", rendered)
end

@testset "Multiple HTMX.HyperscriptString children concatenate in _ attribute" begin
